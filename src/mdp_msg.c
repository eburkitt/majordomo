...
#define GET_OCTETS(host,size) { \
    if (self->needle + size > self->ceiling) { \
        zsys_warning ("mdp_msg: GET_OCTETS failed"); \
        goto malformed; \
    } \
...
#define GET_NUMBER1(host) { \
    if (self->needle + 1 > self->ceiling) { \
        zsys_warning ("mdp_msg: GET_NUMBER1 failed"); \
        goto malformed; \
    } \
...
#define GET_NUMBER2(host) { \
    if (self->needle + 2 > self->ceiling) { \
        zsys_warning ("mdp_msg: GET_NUMBER2 failed"); \
        goto malformed; \
    } \
...
#define GET_NUMBER4(host) { \
    if (self->needle + 4 > self->ceiling) { \
        zsys_warning ("mdp_msg: GET_NUMBER4 failed"); \
        goto malformed; \
    } \
...
#define GET_NUMBER8(host) { \
    if (self->needle + 8 > self->ceiling) { \
        zsys_warning ("mdp_msg: GET_NUMBER8 failed"); \
        goto malformed; \
    } \
...
#define GET_STRING(host) { \
    ...
    if (self->needle + string_size > (self->ceiling)) { \
        zsys_warning ("mdp_msg: GET_STRING failed"); \
        goto malformed; \
    } \
...
#define GET_LONGSTR(host) { \
    ...
    if (self->needle + string_size > (self->ceiling)) { \
        zsys_warning ("mdp_msg: GET_LONGSTR failed"); \
        goto malformed; \
    } \
    free ((host)); \
...
//  --------------------------------------------------------------------------
//  Create a new mdp_msg

mdp_msg_t *
mdp_msg_new (void)
{
    mdp_msg_t *self = (mdp_msg_t *) zmalloc (sizeof (mdp_msg_t));
    return self;
}
...
mdp_msg_recv (mdp_msg_t *self, zsock_t *input)
{
    ...
    if (zsock_type (input) == ZMQ_ROUTER) {
        ...
        if (!self->routing_id || !zsock_rcvmore (input)) {
            zsys_warning ("mdp_msg: no routing ID");
    ...
    if (size == -1) {
        zsys_warning ("mdp_msg: interrupted");
    ...
    if (signature != (0xAAA0 | 4)) {
        zsys_warning ("mdp_msg: invalid signature");
    ...
    switch (self->id) {
        case MDP_MSG_CLIENT_REQUEST:
            {
                char version [256];
                GET_STRING (version);
                if (strneq (version, "MDPC02")) {
                    zsys_warning ("mdp_msg: version is invalid");
                    goto malformed;
                }
            }
            {
                ...
                if (messageid != 1) {
                    zsys_warning ("mdp_msg: messageid is invalid");
                    goto malformed;
                }
            }
        ...
        case MDP_MSG_WORKER_REQUEST:
            ...
            if (!zsock_rcvmore (input)) {
                zsys_warning ("mdp_msg: address is missing");
                goto malformed;
            }
            zframe_destroy (&self->address);
            ...
            //  Get next frame off socket
            if (!zsock_rcvmore (input)) {
                zsys_warning ("mdp_msg: empty is missing");
                goto malformed;
            }
            zframe_destroy (&self->empty);
        ...
        default:
            zsys_warning ("mdp_msg: bad message ID");
            goto malformed;
    }
    //  Successful return
    zmq_msg_close (&frame);
    ...
    //  Error returns
    malformed:
        zsys_warning ("mdp_msg: mdp_msg malformed message, fail");
        zmq_msg_close (&frame);
...
//  --------------------------------------------------------------------------
//  Send the mdp_msg to the socket. Does not destroy it. Returns 0 if
//  OK, else -1.
...
mdp_msg_send (mdp_msg_t *self, zsock_t *output)
{
    ...
    if (self->id == MDP_MSG_WORKER_REQUEST) {
    ...
    //  Now send any frame fields, in order
    if (self->id == MDP_MSG_WORKER_PARTIAL) {
        //  If address isn't set, send an empty frame
        if (self->address)
            zframe_send (&self->address, output, ZFRAME_REUSE + (--nbr_frames? ZFRAME_MORE: 0));
        else
    ...
    //  Now send any frame fields, in order
    if (self->id == MDP_MSG_WORKER_FINAL) {
        //  If address isn't set, send an empty frame
        if (self->address)
            zframe_send (&self->address, output, ZFRAME_REUSE + (--nbr_frames? ZFRAME_MORE: 0));
        else
...
mdp_msg_print (mdp_msg_t *self)
{
    ...
    switch (self->id) {
        case MDP_MSG_CLIENT_REQUEST:
            zsys_debug ("MDP_MSG_CLIENT_REQUEST:");
        ...
        case MDP_MSG_CLIENT_PARTIAL:
            zsys_debug ("MDP_MSG_CLIENT_PARTIAL:");
            zsys_debug ("    version=mdpc02");
        ...
        case MDP_MSG_CLIENT_FINAL:
            zsys_debug ("MDP_MSG_CLIENT_FINAL:");
            zsys_debug ("    version=mdpc02");
        ...
        case MDP_MSG_WORKER_REQUEST:
            zsys_debug ("MDP_MSG_WORKER_REQUEST:");
            zsys_debug ("    version=mdpw02");
            zsys_debug ("    messageid=2");
            zsys_debug ("    address=");
            if (self->address)
        ...
        case MDP_MSG_WORKER_PARTIAL:
            zsys_debug ("MDP_MSG_WORKER_PARTIAL:");
            zsys_debug ("    version=mdpw02");
            zsys_debug ("    messageid=3");
            zsys_debug ("    address=");
            if (self->address)
        ...
        case MDP_MSG_WORKER_FINAL:
            zsys_debug ("MDP_MSG_WORKER_FINAL:");
            zsys_debug ("    version=mdpw02");
        ...
        case MDP_MSG_HEARTBEAT:
            zsys_debug ("MDP_MSG_HEARTBEAT:");
            zsys_debug ("    version=mdpw02");
            zsys_debug ("    messageid=5");
            break;
            
        case MDP_MSG_DISCONNECT:
            zsys_debug ("MDP_MSG_DISCONNECT:");
            zsys_debug ("    version=mdpw02");
